...
int correspondeIniciarSemaforo(void)
{

    if(!abrirArchivo("flag.txt", "r"))
    {
        /* No se puede abrir, o sea que es el primer proceso ejecutado... */
        /* Entonces creamos el archivo... */

        if(!abrirArchivo("flag.txt", "w"))
        {
            printf("\nError al abrir archivo archivo flag...\n");
        }

        cerrarArchivo();
        return 1; /* SI, CORRESPONDE */
    } else
    {
        /* Se pudo abrir, entonces no es el primero */
        cerrarArchivo();
        return 0; /* NO, NO CORRESPONDE */
    }

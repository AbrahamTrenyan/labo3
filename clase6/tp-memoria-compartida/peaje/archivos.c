...
int inEscribirArchivo(char *szNombreArchivo, char *szDatos, char *szModo)
{
    if (inAbrirArchivo(szNombreArchivo, szModo))
    {
...
int inEliminarArchivo(char *szNombreArchivo)
{
    if (remove(szNombreArchivo) == 0)
    {
        return TRUE;
    }
    return FALSE;
